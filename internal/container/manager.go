...
func cloneContainer(c *Container) *Container {
	x := *c
	x.Spec.Cmd = append([]string(nil), c.Spec.Cmd...)
	x.Spec.Env = append([]string(nil), c.Spec.Env...)
	x.Spec.Mounts = append([]MountSpec(nil), c.Spec.Mounts...)
	x.Mounts = append([]MountRecord(nil), c.Mounts...)
	if c.Net != nil {
		v := *c.Net
		x.Net = &v
	}
...
func (m *Manager) Create(o CreateOptions) (*Container, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("%v: %v", ErrImageMissing, err)
	}
	o.Image = img.Name
	m.mu.Lock()
